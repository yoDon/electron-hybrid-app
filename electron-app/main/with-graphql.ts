...
ipcMain.on("w2m-getApiDetails", (event:Electron.Event) => {
    if (apiDetails.signingKey !== "") {
        event.sender.send("m2w-apiDetails", JSON.stringify(apiDetails));
    } else {
        initializeApi()
            .then(() => {
                event.sender.send("m2w-apiDetails", JSON.stringify(apiDetails));
            ...
            .catch(() => {
                event.sender.send("m2w-apiDetailsError", "Error initializing API");
            });
    }
});
